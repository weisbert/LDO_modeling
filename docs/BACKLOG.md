...
- ~~**[MAJOR] Auto-derive the unload-discharge transparency floor `ovVdz` from the MAX characterized large-signal ripple**~~ — **DONE (2026-07-01).** `harness/fit_ovvdz.derive_ovvdz`, folded into `fit_multiport` after the LTI fit (PURE-PYTHON, NO sim — mirrors `derive_iassist`): `ovVdz = clamp(2.5·max_f|Zout|·di_ripple [+ opt-in Σvout_amp_k + max|H_psrr|·vrip], [5 mV, 0.5·(supply−vreg)])`, `di_ripple = 0.10·I_op` (the PERIODIC-ripple di, `I_op` = coverage.transient base; NOT the big assist step). SCOPE was the FLOOR value only (gate mechanism settled — heavy/state gate refuted). Reproduces the former hardcoded 25 mV as the WuR case end-to-end through emit (**PLL 24.64 mV** = 2.5·50 µA·197.1 Ω, **VCO 26.07 mV** = 2.5·200 µA·52.1 Ω; spur/PSRR = 0, WuR has neither). `emit_pmu_model._OVD_VDZ`=25 mV is now the FALLBACK (no characterized Zout → emit byte-identical); a manual `vfit['unload_discharge']['ovVdz']` (or manifest rail) still WINS. Composite AC/DC/PSRR/noise UNCHANGED (ovVdz OP-inert). `harness/test_ovvdz_fit.py` (13 cases). See thread `large-signal-recovery` §3 + DATA §5 + METHODOLOGY §Large-signal. (Per-corner ovVdz + wiring into pvt scheme ② remains an optional stretch.)
