...
- **[MAJOR] Auto-derive the unload-discharge transparency floor `ovVdz` from the MAX characterized large-signal ripple** (the planned NEXT step, 2026-07-01). Today `ovVdz`=25 mV is HARDCODED (`emit_pmu_model._OVD_VDZ`), hand-tuned for THIS chip (≈2.5× the ~10 mV documented large-signal ripple, ≪ the 160 mV supply headroom). It is the "large-signal transparency floor": the dump must stay inert up to the largest legitimate rail excursion or it clips the spur/PSRR/Zout SPECTRA (Spectre: 16 mV distortion at ovVdz=3 mV). For a DIFFERENT LDO (bigger ripple / smaller headroom / different L) 25 mV can be wrong. GOAL: extract the max ABOVE-vreg large-signal rail excursion in `fit_multiport` and set `ovVdz = k·that`, clamped to `[DC-droop floor, α·(supply−vreg)]`; per-corner if it varies (ties into pvt scheme ②). **SCOPE-FIRST — the gating unknown:** WHERE does the above-vreg large-signal ripple live in the npz/coverage? Small-signal AC + the loading `tr_*` dips do NOT capture it; it needs a large-signal PSRR/spur characterization. If that data is absent, this is a COVERAGE/char change, not just a fit change — decide that before building. Fallback to the current default (or a `supply−vreg` headroom heuristic) when no such data exists (like the `iassist` seed). Keep the per-rail `vfit['unload_discharge']` manual knob. (From the ovVdz adversarial-review finding; see thread `large-signal-recovery` + METHODOLOGY §Refuted "small ovVdz".)
