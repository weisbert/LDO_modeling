...
- **[MAJOR] PVT route-A execution → now an ACTIVE thread** (`docs/threads/pvt-modeling.md`). Local screen (2026-06-30, 2 transistor GTs) proved a single-corner model is silently OPTIMISTIC at stressed corners (hot I-ceiling 8.5× over-claim; SS/low-V PSRR +11 dB over-claim; near-instability the TT model can't see) and empirically confirmed route-A `.lib` sections + the interpolation REJECTION. Open build items: corner-keyed `.lib` SECTIONS emit (UN-implemented), corner-aware assist `iaG` (the signed-dangerous param), envelope guard. The VDD axis below is subsumed by this thread.
- **[MAJOR] VDD axis (R3-L2)** — small-signal Zout/PSRR/noise at ≥2 vin + line-reg slope; wire the unused `dc_linereg` into the DC anchor so the rail tracks supply. Needs per-supply-corner exports. (Now part of `docs/threads/pvt-modeling.md` — the screen showed supply moves I-ceiling −44 % and PSRR to −1.8 dB at 0.95 V.)
...
- **[MAJOR] Auto-derive the unload-discharge transparency floor `ovVdz` from the MAX characterized large-signal ripple** (the planned NEXT step, 2026-07-01). Today `ovVdz`=25 mV is HARDCODED (`emit_pmu_model._OVD_VDZ`), hand-tuned for THIS chip (≈2.5× the ~10 mV documented large-signal ripple, ≪ the 160 mV supply headroom). It is the "large-signal transparency floor": the dump must stay inert up to the largest legitimate rail excursion or it clips the spur/PSRR/Zout SPECTRA (Spectre: 16 mV distortion at ovVdz=3 mV). For a DIFFERENT LDO (bigger ripple / smaller headroom / different L) 25 mV can be wrong. GOAL: compute the max ABOVE-vreg PERIODIC rail excursion in `fit_multiport` and set `ovVdz = k·that`, clamped to `[DC-droop floor, α·(supply−vreg)]`; per-corner if it varies (ties into pvt scheme ②). **DATA SUFFICIENCY — RESOLVED 2026-07-01: local data IS enough; this is a FIT change, NOT a coverage change.** The excursion is COMPUTABLE from what the npz already holds: `max_f |Zout(f)|·di_ripple` (Zout local; di from coverage) + `Σ vout_amp_k` (spur amps where characterized — each IS a pre-measured above-vreg tone) + `|H_psrr(f)|·vrip`. The hand-set 25 mV is exactly `2.5 × (50 µA coverage-lin di × ~200 Ω Zout plateau) = 2.5 × 10 mV` — auto-fit just mechanizes that hand calc, NO new sim/char. Remaining work is DEFINITIONAL: use the PERIODIC-ripple di (coverage LIN ±50 µA), NOT the one-time BIG step (±500 µA/±1 mA — a transient the assist owns, ×200 Ω would blow the ~160 mV headroom). The one unknowable (customer deployment ripple beyond the characterized envelope) → margin `k` + the per-rail `vfit['unload_discharge']` manual knob, never data. (From the ovVdz adversarial-review finding; see thread `large-signal-recovery` + METHODOLOGY §Refuted "small ovVdz".)
- ~~**[MINOR] UNLOAD overshoot-above-supply**~~ — **DONE (Route 1 branch-A discharge, 2026-07-01).** A high-side/output clamp CAN'T win the trade-off (bounds overshoot only by hanging at the ceiling ~4.7 µs); the fix DRAINS the fit-inductor instead — a source-gated, voltage-bounded reverse EMF in the branch-A regulation (`emit_pmu_model._ovd_*`, DEFAULT-ON). PLL +302 mV(>supply)/2 µs → **+34 mV(≤supply)/monotone**; AC/dip bit-identical; +40 mA pinned. Adversarial-review refinement: `ovVdz` is a LARGE-SIGNAL TRANSPARENCY FLOOR (25 mV, above the ~10 mV char) so the dump stays inert in the spur/PSRR/Zout regime — sub-µs recovery (needs ovVdz<5 mV) was traded for that. See thread `large-signal-recovery` + DATA §5 + METHODOLOGY §Large-signal / §Refuted.
