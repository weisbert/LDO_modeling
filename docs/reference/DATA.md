...
- **Heavy-tier (memory/state) discharge gate — REFUTED (reduced PLL branch-A, local Spectre, 4mA→0.3mA/1ns/20pF, vreg 0.82 / supply 0.98).** Reduced model validation: shipped ovVdz=25 mV → **+35.3 mV peak / ≤supply / 1.84 µs** (matches the full model's +34.3 mV — the gated result is insensitive to node cap). Bare +593 mV(>supply; deeper than the full model's +302 mV only because the reduced node cap is 25 pF). Gate comparison **[peak / >supply? / rec-to-5mV / clip@2MHz / clip@20MHz]**: shipped ovVdz=25 mV **[+35 mV / no / 1.84 µs / 0.000 / 0.000]**; shipped ovVdz=3 mV [+14 mV / no / 0.63 µs / 16.85 / 16.36]; heavy qdz=3m τ=200n [+432 mV / **YES** / 0.42 µs / 18.79 / 0.000]; heavy qdz=1m τ=100n [+298 mV / YES / 0.23 µs / 22.71 / 0.49]; heavy qdz=1m τ=50n [+225 mV / YES / 0.20 µs / 18.83 / 8.60]. All AC modes bit-identical vs bare (0.000 dB). ⇒ heavy fails peak-bound (memory lag) AND 2 MHz transparency; shipped instantaneous gate is Pareto-optimal. Reduced-model TB `scratchpad/heavy/` (not committed). Verdict: METHODOLOGY §Refuted.
