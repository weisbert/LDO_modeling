...
- **Compressive branch-A current-assist `i_assist=f(verr)`** (f odd/compressive, f'(0)=0 exact → AC bit-identical at OP; 2 params fit Spectre-in-loop, held-out across amplitudes) — **OPEN/PROPOSED** — the right fix for the GT sub-linear stiffening dip. Awaiting user scope decision.
