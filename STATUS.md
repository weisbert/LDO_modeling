...
|---|---|---|---|
| large-signal-recovery | PART2 compressive assist (`78f5a7a`) + **Route 1 unload-discharge SHIPPED** (fit-inductor drain: +302 mV(>supply)→+34 mV(≤supply), AC/dip bit-identical, +40 mA pinned, survives minimal; ovVdz=25 mV transparency floor). **Heavy-tier (memory/state) gate REFUTED** (local Spectre: can't bound the ns peak [memory lag → +225…432 mV >supply] AND freq-limited transparency [18.8 mV clip @2 MHz] → shipped instantaneous gate is Pareto-optimal; sub-µs recovery is fundamental, gyrator won't help) | `bash apply` + box re-validate; **then (user-directed next round) auto-derive `ovVdz` — fit the FLOOR of the shipped gate, don't replace the gate** | `docs/threads/large-signal-recovery.md` |
