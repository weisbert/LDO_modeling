...
**Next: `bash apply` + box re-validate** (the box must regenerate the .va WITH the assist+floor, not the stale minimal box.va). Optional: commit the local validation TBs in `cadence/wur_real_tb/` (UNCOMMITTED: `PMU_model{,_old,_floor}.va`, `iload_PLL_delayed10ns.txt`, `drive_pll_{delayed,step600to300,combo}.scs` — a full-PMU load-step/replay harness driving the .va with split decap + ipwl); VCO real-iload replay; unload overshoot clamp; T55 z re-export for the ~×0.65 T-confound.
