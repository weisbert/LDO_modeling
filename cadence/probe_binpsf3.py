...
def main():
    ...
    # ---- decode time + target for BOTH layouts, all points, all buffers (data_start + w*bufsz) ----
    def sig_off(buf, signal):
        return data + buf * bufsz + signal * wsz

    def series(signal):
        out = []
        for i in range(npts):
            bf, j = divmod(i, w)
            out.append(f64(b, sig_off(bf, signal) + j * 8))
        return out

    layouts = {
        "sweep-FIRST (sweep=sig0, trace k=sig 1+k)": (0, 1 + k),
        "sweep-LAST  (trace k=sig k, sweep=sig N)": (ntr, k),
    }
    print("\n--- decode time + target for each layout (the one with CLEAN time 0->tmax wins) ---")
    for label, (sw_sig, tr_sig) in layouts.items():
        t = series(sw_sig)
        vk = series(tr_sig)
        t_ok = (t[0] is not None and abs(t[0]) < 1e-12
                and all(t[i] is not None and t[i] >= t[i - 1] for i in range(1, npts))
                and t[-1] is not None and t[-1] <= tmax * 1.0001)
        print(f"\n  [{'TIME-OK <<<' if t_ok else 'time bad'}] {label}")
        print(f"     time[0..6]   = {[fmt(x) for x in t[:7]]}")
        print(f"     time[-4:]    = {[fmt(x) for x in t[-4:]]}  (want 0 .. ~{tmax:g})")
        print(f"     {want}[0..6]  = {[fmt(x) for x in vk[:7]]}")
        print(f"     {want}[-4:]   = {[fmt(x) for x in vk[-4:]]}")
