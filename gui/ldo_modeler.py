    ...
    class MainWindow(QMainWindow):
        ...
        def _render_report_current(self, view):
            ...
            if "iv" in present:
                ...
                # absolute µA on the ticks (no '+1.50' offset trap) + an EXPLICIT 0-based range
                # from the data so a flat source reads at its true level. set both limits, not
                # set_ylim(bottom=0) which freezes the top at a stale value on older matplotlib
                # (box) and clips the line off-screen.
                _ys = np.concatenate([np.asarray(vw["iv_i"]), np.asarray(mo["iv"])]) * 1e6
                _lo, _hi = min(0.0, float(_ys.min())), max(0.0, float(_ys.max()))
                if _hi == _lo: _hi = _lo + 1.0
                a.set_ylim(_lo, _hi + 0.12 * (_hi - _lo)); a.ticklabel_format(useOffset=False, axis="y")
            else:
        ...
        def _draw_current_compare(self, pin):
            ...
            # no '+1.50' offset trap; EXPLICIT 0-based range from the data (set both limits, not
            # set_ylim(bottom=0) which clips the line on older matplotlib) -> true µA level shows.
            _ys = np.concatenate([np.asarray(v["iv_i"]), np.asarray(d["iv_model"])]) * 1e6
            _lo, _hi = min(0.0, float(_ys.min())), max(0.0, float(_ys.max()))
            if _hi == _lo: _hi = _lo + 1.0
            ax[0, 0].set_ylim(_lo, _hi + 0.12 * (_hi - _lo)); ax[0, 0].ticklabel_format(useOffset=False, axis="y")
