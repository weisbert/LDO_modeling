...
```
<你建的文件夹>/              # 如 ~/LDO_modeling（= PREFIX 本身；cd 进来后 $PWD 就是它）
  ldo_modeler_full.tar.gz    # 传过来的完整包（装完可删）
...
**PREFIX 必须用绝对路径**——bootstrap 会建 `app/results → results` 软链，相对路径会让软链指错位置。bootstrap 从不删 PREFIX 本身、`update.sh` 只删 `app/`，所以 PREFIX 直接用这个文件夹（`$PWD`）是安全的，tar 包/`bundle/` 不受影响。
...
### 1.1 红区（已部署后，日常使用）
```bash
cd /path/to/你的文件夹          # 安装时那个文件夹（如 ~/LDO_modeling）
./run_gui                       # 启动器（bootstrap 自动生成）：用自带 Qt，绕开机器上 Cadence/系统的 Qt；需有显示(X11/VNC)
# 想直接载入一份已有参考数据看效果：
./run_gui --ref results/<name>.npz
```
> `run_gui` 把 venv 自带的 Qt 5.15.2（`PyQt5/Qt5/lib`）前置到 `LD_LIBRARY_PATH`。EDA 机器上常有 Cadence/系统的
> `libQt5Core.so.5`（如 `/software/public/qt/5.15.3_xcb/lib`）会遮挡它、报 `symbol ... version Qt_5 not defined`。
...
bash bundle/bootstrap.sh "$PWD"              # PREFIX = 这个文件夹本身 → .venv/app/results 直接装在它下面；bash 调用免 +x
```
> 不传参数时 bootstrap 默认 `/opt/ldo_modeler`——共享服务器一般写不了，所以**显式传一个可写的绝对路径**。这里用 shell 内置的 `$PWD`（= 当前文件夹，不新建任何变量、零冲突）；嫌它"魔法"就把绝对路径打全。装完 `bundle/` 和 tar 包可删。
`bootstrap.sh` 五步：
...
```bash
cd /path/to/你的文件夹                       # 安装目录（增量 tar 已上传到这里）
./update                                     # 一条命令：校验+解包+换 app/+冒烟（bootstrap 自动生成的启动器）

# 若该安装是早期版本、没有 ./update 启动器，用这条手动等价的一行（功能相同）：
#   mkdir -p bundle_incr && tar xzf ldo_modeler_incremental.tar.gz -C bundle_incr && bash bundle_incr/update.sh "$PWD"
```
`update.sh` 做的事：
...
| 红区 `python3.11 not found` | 装 python3.11，或 `PYTHON=/abs/path/python3.11 bash bundle/bootstrap.sh "$PWD"`。 |
| `mkdir: cannot create '/opt/...'：Permission denied` | PREFIX 没写权限。在你建的文件夹里执行，换可写的**绝对**路径：`bash bundle/bootstrap.sh "$PWD"`。 |
| `sha256sum -c` 报 `No such file`/文件名带 `\r` | 旧 Windows 包边车是 CRLF：`sed 's/\r$//' *.sha256 \| sha256sum -c`。新包已写 LF。 |
| `app/results` 成了坏链 / 启动找不到数据 | bootstrap 的 PREFIX 用了相对路径。重跑，PREFIX 用绝对路径 `"$PWD"`。 |
...
| 纯计算节点 bootstrap 冒烟测试因缺 Qt `.so` 失败 | `SMOKE_REQUIRE_QT=0 bash bundle/bootstrap.sh "$PWD"` 跳过强制 Qt（离线装包与 numpy/scipy 仍照常验证）。 |
| 启动/自检报 `symbol _ZdaPvm, version Qt_5 not defined in file libQt5Core.so.5` | 机器上的 Cadence/系统 Qt 盖住了自带 Qt 5.15.2。用 `./run_gui`（已内置隔离）；bootstrap/update 的冒烟也已自动前置自带 Qt（`.venv/lib/python3.11/site-packages/PyQt5/Qt5/lib`）。 |
...
bash bundle/bootstrap.sh "$PWD"                                    # 初次部署（FULL），PREFIX = 这个文件夹
bash bundle_incr/update.sh "$PWD"                                  # 增量更新（先把增量包解到 bundle_incr/）
./run_gui                                                          # 启动 GUI（自带 Qt 隔离；需显示）
```
